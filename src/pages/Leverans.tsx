...
const saveState = (s: Record<string, boolean>) =>
    ...
    items: [
      {
        ...
        linkHref:
          "https://docs.google.com/spreadsheets/d/16EDRcYErHPbd4tNv6Jp6Ts3cekBDY7YQl0Zsz4pH69M/edit?usp=sharing",
      },
      {
...
const Card: React.FC<
  React.PropsWithChildren<{ title?: string; sub?: string }>
> = ({ title, sub, children }) => (
  <section
    style={{
  ...
  >
    {(title || sub) && (
      <header style={{ padding: 12, borderBottom: `1px solid ${tokens.border}` }}>
        {sub && (
          <div style={{ color: tokens.muted, fontSize: 12, marginBottom: 2 }}>
            {sub}
          </div>
        )}
    ...
    <div style={{ flex: 1 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        {item.linkHref && item.linkLabel && (
          <LinkBtn href={item.linkHref} label={item.linkLabel} />
        )}
      </div>
      {item.note && (
...
export default function Leverans() {
  ...
  const sendEmailAndClear = () => {
    ...
    const mailto = `mailto:claes@astadvingard.se?subject=${encodeURIComponent(
      subject
  ...
  return (
    <div style={{ background: tokens.bg, color: tokens.text, minHeight: "100vh" }}>
      <div style={{ maxWidth: 1160, margin: "0 auto", padding: 16 }}>
        {/* Header: länk överst, rubrik under */}
        <header
          style={{
            marginBottom: 12,
            display: "flex",
        ...
        {/* Utskriftsknapp högerställd, ej i Card */}
              ...
              >
                {sec.items.map((it) => (
                  <CheckboxItem
                    ...
                    item={it}
                    checked={!!state[it.id]}
                    onToggle={() => setChecked(it.id, !state[it.id])}
                  />
                ))}
              </ul>
            </Card>
            ...
            <div style={{ display: "grid", gap: 8 }}>
              ...
              >
                <div style={{ color: tokens.muted, fontSize: 12 }}>
                  Status sparas lokalt i din webbläsare.
                </div>
